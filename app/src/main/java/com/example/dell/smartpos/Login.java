...
import com.example.spos_sdk2.LoginData;
import com.example.spos_sdk2.LoginRequest;
import com.example.spos_sdk2.LoginResponse;
import com.example.spos_sdk2.LoginResult;
import com.example.spos_sdk2.PayMethod;
...
//                    loginData.setActivity(Login.this);
//                    loginData.setPayGate(EnvBase.PayGate.PAYDOLLAR);
//
//                    // Declare LoginRequest
...
//                                String apiId = loginResult.getAdminId();
...
//                            progressDialog.cancel();
//                            Toast.makeText(Login.this, errorResult.getErrCode() + " - " + errorResult.getErrMessage(),
//                                    Toast.LENGTH_LONG).show();
//                        }
//                    });

                    LoginData loginData = new LoginData();
                    loginData.setMerchantId(merchantID);
                    loginData.setUserId(userID);
                    loginData.setPassword(password);
                    loginData.setActivity(Login.this);
                    ...
                    if (getPrefPayGate().equalsIgnoreCase(Constants.pg_paydollar)) {
                        ...
                        loginData.setPayGate(EnvBase.PayGate.SIAMPAY);
                    } else if (getPrefPayGate().equalsIgnoreCase(Constants.pg_pesopay)) {
                        loginData.setPayGate(EnvBase.PayGate.PESOPAY);
                    }

                    LoginRequest request = new LoginRequest();
                    request.setLoginData(loginData);
                    request.process();
                    request.responseHandler(new LoginResponse() {
                        @Override
                        public void getResponse(LoginResult loginResult) {
                            ...
                            int resultCode = loginResult.getResultCode();
                            Log.d("LoginReq", String.valueOf(resultCode));
                            if(resultCode == SUCCESS) {
                                String currCode = loginResult.getCurrencyCode();
                                String merName = loginResult.getMerchantName();
                                ...
                                Iterator<PayMethod> iter = payMethodList.iterator();
                                while (iter.hasNext()) {
                                    PayMethod pMethod = iter.next();
                                    sb.append(pMethod.getName());
                                    sb.append(",");
                                }

                                String payMethod = sb.toString();
                                String channelType = Joiner.on(",").join(channelTypeList);
                                ...
                                String rate = String.valueOf(rateDbl);
                                String fixed = String.valueOf(fixedDbl);
//
                                login(currCode,
                                        merName,
                                        payMethod,
                                ...
                                showLoginError(loginResult.getReturnMsg());
                            }
                        }
                        ...
                        public void onError(ErrorResult errorResult) {
                            progressDialog.cancel();
                            Toast.makeText(Login.this, errorResult.getErrMessage(),
                                    Toast.LENGTH_LONG).show();
                        }
                    });

//                    PayData data = new PayData();
//                    data.setActivity(Login.this);
//
//                    PaymentSDK paymentSDK = new PaymentSDK();
//                    paymentSDK.setPayData(data);
//                    paymentSDK.process();
//                    paymentSDK.responseHandler(new PaymentResponse() {
//                        @Override
//                        public void getResponse(PayResult payResult) {
//                            String message = payResult.getReturnMsg();
//                            Log.d("Message Return: ", message);
//                        }
//                    });
//                    HistoryData data = new HistoryData();
//                    data.setActivity(Login.this);
//
//                    HistorySDK historySDK = new HistorySDK();
//                    historySDK.setHistoryData(data);
//                    historySDK.process();
//                    historySDK.responseHandler(new HistoryResponse() {
//                        @Override
//                        public void getResponse(ArrayList<Record> arrayList) {
//                            Log.d("Message Return: ", arrayList.toString());
//                        }
//                    });
//
    ...
    public void autoLogin(long timeExpiry) {
        SharedPreferences pref = getApplicationContext().getSharedPreferences(
                Constants.SESSION_DATA, MODE_PRIVATE);
        if (pref.getBoolean("user_logged_in", false)) {
...
//        }
     ...
     * @param enableSMS
     */
    public void login(String Currcode, String merName, String payMethod,
...
//                encPayMethod = encrypter.encrypt(payMethod+",ALIPAYTHOFFL");
            ...
            edit.putString(Constants.pref_CurrCode, encCurrCode);
    ...
    public void initLanguageOption() {
        ...
        spinLang.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View arg1, int pos,
                ...
                if (spinFlag == true) {
                    SharedPreferences pref = getApplicationContext()
                            .getSharedPreferences(Constants.SETTINGS,
                                    MODE_PRIVATE);
                    SharedPreferences.Editor edit = pref.edit();
                    edit.putString(Constants.pref_Lang, selLang);
                    edit.commit();
                    changeLang(selLang);
                    restartActivity();
                    return;
