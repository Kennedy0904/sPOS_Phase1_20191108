...
public class DialogSendReceipt extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_dialog_send_receipt);
		...
		user = sendReceipt.getStringExtra("user");
		password = sendReceipt.getStringExtra("password");
		payRef = sendReceipt.getStringExtra(Constants.PAYREF);
		...
		final EditText edtSMScountry = (EditText)findViewById(R.id.edtSMScountry);
		final EditText edtSMS = (EditText)findViewById(R.id.edtSMS);
		...
		edtSMScountry.setHint(getCountryCode);
		...
		if(!isSmsEnabled(encMerchantId)){
			RelativeLayout relSMS = (RelativeLayout)findViewById(R.id.relaSMS);
			relSMS.setVisibility(View.GONE);
		}
		db.close();
		...
		final TextView verifyMobile = (TextView)findViewById(R.id.verifyMobile);
		...
		inputConfirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if(edtSMS.getText().toString().trim().equals("")&& edtEmail.getText().toString().trim().equals("")){
					Toast.makeText(DialogSendReceipt.this, getString(R.string.pls_mobile_email), Toast.LENGTH_SHORT).show();
				}else{
					String Ccode = edtSMScountry.getText().toString().trim();
					if((Ccode.equals(""))){
						Ccode = edtSMScountry.getHint().toString();
					}else{
						Ccode = edtSMScountry.getText().toString();
					}
					
					if(!edtSMS.getText().toString().equals("")){
						gMobile = Ccode + edtSMS.getText().toString();
					}
					...
					//For Verify
					if(!gMobile.equals("")){
						verifyMobile.setText(getString(R.string.confirm_mobile_no) + gMobile);
		...
		verifyConfirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				SendReceiptTask sendNotif = new SendReceiptTask(DialogSendReceipt.this, getPrefPayGate());
				sendNotif.execute(merId,user, password,  gMobile, gEmail, payRef);
	...
	public class SendReceiptTask extends AsyncTask<String, Void, String> {
		...
		private DialogSendReceipt activity;
		...
		public SendReceiptTask(DialogSendReceipt activity, String payGate)
		{
		...
		protected String doInBackground(String... arg0) {
		    ...
		    try{
		    	baseUrl2 = PayGate.getURL_sendNoti(payGate);
