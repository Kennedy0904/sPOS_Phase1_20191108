...
import com.example.spos_sdk2.TxnData;
import com.example.spos_sdk2.TxnRequest;
import com.example.spos_sdk2.TxnResponse;
import com.example.spos_sdk2.TxnResult;
...
//                txnData.setMerchantId("123456");
//                txnData.setPayRef("123456");
//                txnData.setApiId("apiadmin");
//                txnData.setApiPassword("apipassword");
//                txnData.setActionType(EnvBase.TxnAction.REFUND);
//                txnData.setActivity(DialogActivity.this);
//                txnData.setPayGate(EnvBase.PayGate.PAYDOLLAR);
//
//                // Initialize TxnRequest
//                TxnRequest txnRequest = new TxnRequest();
//                txnRequest.setTxnData(txnData);
//                txnRequest.process();
//
//                // Initialize TxnResponse
//                txnRequest.responseHandler(new TxnResponse() {
//                    @Override
...
//                        Toast.makeText(DialogActivity.this, errorResult.getErrCode() + " - " + errorResult.getErrMessage(), Toast.LENGTH_LONG).show();
//                    }
//                });

                TxnData txnData = new TxnData();
                txnData.setMerchantId(merchantId);
//                txnData.setAmount(edtnewAmt.getText().toString());
                txnData.setPayRef(payRef);
                txnData.setApiId(user);
                txnData.setApiPassword(password);

                if(actionType.equalsIgnoreCase(actOnlineRefund)){
                    txnData.setActionType(EnvBase.TxnAction.REFUND);
                }else if(actionType.equalsIgnoreCase(actOnlinePartialRefund)){
                    txnData.setActionType(EnvBase.TxnAction.PARTIAL_REFUND);
                    txnData.setAmount(edtnewAmt.getText().toString());
                } else if(actionType.equalsIgnoreCase(actVoid)){
                    txnData.setActionType(EnvBase.TxnAction.VOID);
                }
                ...
                if (getPrefPayGate().equalsIgnoreCase(Constants.pg_paydollar)) {
                    ...
                    txnData.setPayGate(EnvBase.PayGate.SIAMPAY);
                } else if (getPrefPayGate().equalsIgnoreCase(Constants.pg_pesopay)) {
                    txnData.setPayGate(EnvBase.PayGate.PESOPAY);
                }

                TxnRequest request = new TxnRequest();
                request.setTxnData(txnData);
                request.process();
                request.responseHandler(new TxnResponse() {
                    @Override
                    public void getResponse(TxnResult result) {
...
//                        String orderStatus = result.getOrderStatus();
                        String errMessage = result.getReturnMsg();

                        if (resultCode == SUCCESS) {

                            success(Double.toString(refundAmt), null, amount);

                        } else {
                            showConfirmError(errMessage);
                        }
                    }

                    @Override
                    public void onError(ErrorResult errorResult) {
                        Toast.makeText(DialogActivity.this, errorResult.getErrMessage(),
                                Toast.LENGTH_LONG).show();
                    }
                });
