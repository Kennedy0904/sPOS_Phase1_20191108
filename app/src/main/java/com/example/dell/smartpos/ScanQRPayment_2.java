...
import com.example.spos_sdk2.Payment;
...
//                    payData.setActivity(ScanQRPayment_2.this);
//
...
//                            Toast.makeText(ScanQRPayment_2.this, errorResult.getErrCode() + " - " + errorResult.getErrMessage(), Toast.LENGTH_LONG).show();
//                        }
//                    });


                    progressDialog_ALIPAY.show();
                    PayData payData = new PayData();
                    payData.setMerchantId(merchantId);
                    payData.setAmount(amount);
                    payData.setCurrCode(EnvBase.Currency.valueOf(CurrCode.getName(currCode)));
                    payData.setPayment(EnvBase.Payment.SCAN_QR);
                    payData.setPayType(EnvBase.PayType.NORMAL_PAYMENT);
                    ...
                    try {
                        System.out.println("PaymentReq" + getFieldNamesAndValues(payResult, false));
                    } catch (IllegalAccessException e) {
                        e.printStackTrace();
                    }

//                            successcode=1&Ref=000002&PayRef=7511016&Amt=2.0&Cur=344&prc=&src=&Ord=&Holder=&AuthId=&TId=&MId=2088621894215567&TxTime=null&errMsg=Transaction failed
//                            successcode=1&Ref=000001&PayRef=7510827&Amt=2.0&Cur=344&prc=&src=&Ord=&Holder=ALIPAYOFFL&AuthId=&TId=&MId=2088621894215567&TxTime=null&errMsg=Transaction failed
                            int resultCode = payResult.getResultCode();
                            String merchant_ref_no = payResult.getMerchantRef();
                            String PayRef = payResult.getPayDollarRef();
                            String amount = payResult.getAmount();
                            String MerRequestAmt = payResult.getAmount();
                            String Currcode = payResult.getCurrency();
                            String pMethod = payResult.getPayMethod();
                            String Time = payResult.getTxnTime();
                            String errcode = payResult.getReturnMsg();
                            String cardNo = payResult.getBankRef();
                            ...
                            if(successcode.equalsIgnoreCase("0")){
                                ...
                                scanfail(successcode, merchantId, merName, payType, pMethodAlipay, "admin", cardNo,
                                        merchant_ref_no, PayRef, amount, MerRequestAmt, Surcharge, Currcode, Time, errcode, cardNo);
                            }

                            progressDialog_ALIPAY.cancel();
                        }
                        ...
                        public void onError(ErrorResult errorResult) {
                            ...
                            Toast.makeText(ScanQRPayment_2.this, errorResult.getErrMessage(),
