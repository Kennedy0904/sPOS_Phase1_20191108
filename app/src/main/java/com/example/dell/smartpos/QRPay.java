...
public class QRPay extends AsyncTask<String, Void, String> {
    ...
    public QRPay(ScanQRPayment_2 activity, ProgressDialog progressDialog, String PayGate, boolean retry) {
    ...
    public QRPay(ScanQR_Failure activity, ProgressDialog progressDialog, String PayGate, boolean retry) {
...
//        argSoftwareVersion = arg0[13];
        ...
        if (pMethod.equals("UNIONPAY")) {
            NameValuePair merchantIdNVpair = new BasicNameValuePair("merchantId", argmerchantId);
            NameValuePair loginidNVpair = new BasicNameValuePair("merRequestAmt", argmerRequestAmt);
            NameValuePair passwordNVpair = new BasicNameValuePair("orderRef", argmerchantRef);
            NameValuePair mobileNVpair = new BasicNameValuePair("email", "");
            NameValuePair emailNVpair = new BasicNameValuePair("mobileNo", "");
            NameValuePair payRefNVpair = new BasicNameValuePair("orderId", "");
            NameValuePair payMethodNVpair = new BasicNameValuePair("pMethod", argpmethod);
            NameValuePair merNameNVpair = new BasicNameValuePair("merName", argmerName);
            ...
            NameValuePair cardHolderNVPair = new BasicNameValuePair("cardHolder", argpmethod);
            ...
            if(argpmethod.equalsIgnoreCase("BOOSTOFFL")) {
        ...
        try {
            if (pMethod.equals("UNIONPAY")) {
                baseUrl = PayGate.getURL_CheckStatus(payGate);
            } else if (pMethod.equals("ALIPAYTHOFFL") || pMethod.equals("WECHATHKOFFL")) {
                baseUrl = "https://test.paydollar.com/b2cDemo/eng/directPay/bbl.jsp";
            } else {
            ...
            con.setReadTimeout(240000);// readtimeout:4min  decide by serivce response
    ...
    protected void onPostExecute(String s) {
        ...
        if (GlobalFunction.isQRpMethods(pMethod)) {
            // Change "cardNo" to bankref value if pMethod is QR payment methods
            cardNo = partnerTrxId;
        }
